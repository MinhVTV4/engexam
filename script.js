...
function addMessageToLog(logEl, sender, text) {
    ...
    messageDiv.innerHTML = `
        <div class="flex justify-between items-center mb-1">
            <div class="font-bold text-sm ${senderColor}">${senderName}</div>
            ${sender === 'ai' ? `<button class="speak-message-btn p-1 rounded-full hover:bg-slate-300 transition-colors" data-text-to-speak="${encodedText}" title="Phát lại">${speakIconSVG}</button>` : ''}
        </div>
        <p class="p-3 rounded-lg ${bubbleColor}">${text}</p>
    `;
...
appContainer.addEventListener('click', (e) => {
    ...
    if (speakBtn) {
        playSound('click');
        const textToSpeak = decodeURIComponent(speakBtn.dataset.textToSpeak);
